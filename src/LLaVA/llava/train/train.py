...
class XAIDataset(Dataset):
    ...
    prompt = "What is wrong in the image?"
    use_keypoints = False
    ...
    def __init__(self, split: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
        super(XAIDataset, self).__init__()
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        video_metadata = self.csv_data.loc[i]
        ...
        click_locations_frames_id = [int(t) for t in list(click_locations.keys())]
        
        cap = cv2.VideoCapture(movie_name)
        
        movie_no_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        
        if video_metadata["dataset"] != "Original" and (len(click_locations_frames_id) > 0 and click_locations_frames_id[0] < movie_no_frames - 1):
            # cap.set(cv2.CAP_PROP_POS_FRAMES, click_locations_frames_id[0])
            frame_no = random.sample(click_locations_frames_id, 1)[0]
            while frame_no >= movie_no_frames:
                print(f'{video_metadata["dataset"]}, {movie_name}: {frame_no} / {movie_no_frames}')
                frame_no = random.sample(click_locations_frames_id, 1)[0]
                frame_no = frame_no - int(frame_no == movie_no_frames)

            if str(frame_no) in click_locations:
                keypoint = click_locations[str(frame_no)]
            else:
                keypoint = click_locations[str(frame_no+1)]
        ...
        if self.data_args.image_aspect_ratio == 'pad':
            ...
            preprocessing_args = {"images": image, "return_tensors": "pt"}
            if self.use_keypoints:
                preprocessing_args["keypoints_fn"] = self.apply_mask
                preprocessing_args["keypoints_hard_masking"] = self.use_hard_masking
                preprocessing_args["keypoint"] = keypoint
            image = processor.preprocess(**preprocessing_args)['pixel_values'][0]
        else:
            image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    train_dataset = XAIDataset(tokenizer=tokenizer,
                                split="train",
                                data_args=data_args)
    eval_dataset = XAIDataset(tokenizer=tokenizer,
                                split="val",
                                data_args=data_args)
